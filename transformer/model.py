...
class LayerNormalization(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        #(batch, seq_len, d_model) --> (batch, seq_len, d_model) 对tensor的最后一个维度进行计算，即在每一个样本feature的维度上进行计算
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def attention(query, key, value, mask, dropout: nn.Dropout):
        ...
        return (attention_scores @ value), attention_scores #(batch, k, seq_len, d_k), (batch, h, seq_len, seq_len)
...
class Transformer(nn.Module):
    def __init__(self, encoder: Encoder, decoder: Decoder, src_embed: InputEmbeddings, tgt_embed: InputEmbeddings, 
                 src_pos: PositionEncodeing, tgt_pos: PositionEncodeing, proj_layer: ProjectionLayer):
        super().__init__()
    ...
    def decode(self, tgt, src_mask, tgt_mask, encoder_output):
...
def build_transformer(src_vocab_size: int, tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int, d_model: int = 512,
    ...
    # Create the PositionEncodeing layers
    src_pos = PositionEncodeing(d_model, src_seq_len, dropout)
    tgt_pos = PositionEncodeing(d_model, tgt_seq_len, dropout)
