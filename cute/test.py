...
def main(bs=1, head=64, seq_len=4096, dim=64):
    ...
    base_time = run_benchmark(
        epoch, warmup, self_attention, q, k, v, causal=is_causal, sm_scale=sm_scale
    )
    baseline = self_attention(q, k, v, causal=is_causal, sm_scale=sm_scale)

    flash2_time = run_benchmark(
        epoch, warmup, flash_attention_v2_cutlass, q, k, v, is_causal, sm_scale
    )
    flash2_cutlass_ref = flash_attention_v2_cutlass(q, k, v, is_causal, sm_scale)

    fq = q.transpose(1, 2)
    fk = k.transpose(1, 2)
    fv = v.transpose(1, 2)
    official_ref_time = run_benchmark(
        epoch,
        warmup,
        flash_attn_func_offical,
        fq,
        fk,
        fv,
        causal=is_causal,
    ...
    official_result = flash_attn_func_offical(
        fq, fk, fv, causal=is_causal, softmax_scale=sm_scale
    )

    print(
        f"bs:{bs}, head:{head}, seq_len:{seq_len}, dim:{dim}        \
            baseline:{official_ref_time * 1000 / epoch} ms        \
            flash2_cutlass_fp16:{flash2_time * 1000 / epoch} ms"
    )

    # assert torch.allclose(official_result, flash2_cutlass_ref, rtol=0, atol=1e-2)
